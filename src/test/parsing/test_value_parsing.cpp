...
TEST(ParsingTests, DoubleValueParsing) {
    for (const auto& test_case : double_test_cases) {
        ...
        auto result = RingCompanionHelper<double>::from_string(parse_formula(test_case.formula, Datatype::DYNAMIC, vars, 20), 1.0);
...
TEST(ParsingTests, RationalValueParsing) {
    for (const auto& test_case : rational_test_cases) {
        auto vars = std::map<std::string, std::vector<MathLexerElement>>();
        auto result = RingCompanionHelper<RationalNumber<BigInt>>::from_string(parse_formula(test_case.formula, Datatype::DYNAMIC, vars, 20), BigInt(1));
...
TEST(ParsingTests, ModValueParsing) {
    for (const auto& test_case : mod_test_cases) {
        auto vars = std::map<std::string, std::vector<MathLexerElement>>();
        auto result = parse_modlong_value(parse_formula(test_case.formula, Datatype::DYNAMIC, vars, 20));
