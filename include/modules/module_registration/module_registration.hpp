...
class ModuleConstant {
    std::shared_ptr<SymObjectContainer> value;
 public:
     ModuleConstant(std::shared_ptr<SymObjectContainer> val) : value(val) { }

     std::shared_ptr<SymObjectContainer> get_value() const {
         return value;
     }
};
